...
To evaluate your fine-tuned models against the Q-SART benchmark (assessing structural resilience, state preservation, and economic rationality), use the official [Q-SART Evaluation Suite](https://github.com/AM-2304/Q-SART).

Follow these instructions to run the evaluation pipeline:

1. **Clone the Q-SART Repository**:
   ```bash
   git clone https://github.com/AM-2304/Q-SART.git
   cd Q-SART
   pip install -r requirements.txt
   ```
2. **Download or Link Your Model**:
   Copy or symlink your quantized GGUF model (e.g., `Gemma4NPC-Quest-12B-it.gguf`) into the `Q-SART/models/` directory.
3. **Execute the Evaluation Script**:
   Configure the environment variables (e.g., `HF_TOKEN` if testing Hugging Face endpoints or `GROQ_API_KEY` for API baselines) and run:
   ```bash
   bash run_evaluations.sh
   ```
   For local GGUF models, use the direct Python runner:
   ```bash
   python src/evaluate.py --model local_gguf --model-path models/Gemma4NPC-Quest-12B-it.gguf
   ```
